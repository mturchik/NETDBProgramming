...
{
    public class EnhancementDb : IAccessible
    {
        private static NLog.Logger logger = NLog.LogManager.GetCurrentClassLogger();
        public List<Enhancement> Enhancements { get; set; } = new List<Enhancement>();
        private const string File = "../../Enhancements.csv";

        public EnhancementDb()
        {
            //check for file
            ...
            else
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    Enhancements.Add(new Enhancement(tickInfo.ToArray()));
                }
                sr.Close();
            }
        }
        ...
        public void Remove()
        {
            ...
            foreach (var enhancement in Enhancements)
            {
                if (enhancement.TicketId == int.Parse(input))
                {
                    index = Enhancements.IndexOf(enhancement);
                }
            }
            ...
            else
            {
                ...
                {
                    case "1":
                        logger.Trace("Ticket Deleted, {ticket}", Enhancements[index]);
                        Console.WriteLine("=Ticket Deleted.");
                        Enhancements.RemoveAt(index);
                        StreamWriter sw = new StreamWriter(File);
                        foreach (var enhancement in Enhancements)
                        {
                            sw.WriteLine(enhancement.ToString());
